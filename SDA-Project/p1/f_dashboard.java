...
import p1.C_Notifications.ButtonEditor;
import p1.C_Notifications.ButtonRenderer;
...
public class f_dashboard extends JFrame
{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					f_dashboard frame = new f_dashboard("abc");
	 ...
	 * Create the frame.
	 */
	public f_dashboard(String username) 
	{
		//s=ser;
		un=username;
		setTitle("Dashboard Freelancer");
		...
		panel_2.setBounds(409, 23, 550, 436);
		panel_1.add(panel_2);
		...
		//fetchDataAndDisplayTable(username);
	     if(r.ViewHistory(1, username))
	     {
		 try (Connection connection = DriverManager.getConnection(url, usn, p))
         {
         	String query = "SELECT * FROM bookinghistory WHERE TRIM(Fusername)  = ?";
